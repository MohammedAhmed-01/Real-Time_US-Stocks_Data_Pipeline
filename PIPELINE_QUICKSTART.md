# 🚀 Real-Time US Stocks Data Pipeline — Full Run Guide

> Run the entire pipeline from zero to AI-ready in order. Every command is copy-paste ready.
...
- [ ] Docker Desktop installed (v24+) with **at least 8 GB RAM** allocated
...
| **PostgreSQL** | `localhost:5432` | `stocks` / `stocks123` / db: `stocks_analytics` |
...
Open `.env` and fill in your Kaggle credentials (the values below are placeholders):
...
Leave the rest of `.env` as-is for local development — the MinIO, Kafka, and PostgreSQL defaults work out of the box.
...
> Let this run for **at least 5 minutes** before moving to Step 6, so MinIO has enough data. You can verify data is flowing at **http://localhost:9001** → Buckets → stocks → clean.
...
6. Click **Save** — all 10 tables and 8 views will be visible under Schemas → public.
...
# Full reset — delete all Kafka logs, MinIO data, and PostgreSQL data
...
# Kafka topic initialisation (one-time)
docker compose logs kafka-init

# MinIO bucket creation (one-time)
...
| Problem | Fix |
|---|---|
| Brokers not healthy after `up` | KRaft election takes up to 90s on first boot — wait and re-check |
| `kafka-init` failed | Check `docker compose logs kafka-init` — last line should be `Topics ready. Done.` |
| Producer Kaggle auth error | Verify `KAGGLE_USERNAME` and `KAGGLE_KEY` in `.env` |
| `Cannot read clean data from MinIO` | Step 5 (streaming job) must run first — MinIO starts empty |
| Tables missing in PostgreSQL | Run Step 6 (analytics job) — PostgreSQL starts empty |
| Views missing after analytics | Run Step 7 (post_analytics.sql) — Spark's overwrite drops them |
| Port already in use | Stop the conflicting service or edit the host port in `docker-compose.yml` |
| Spark OOM / crashing | Allocate ≥ 8 GB RAM to Docker in Docker Desktop → Settings → Resources |
...
| Data | Location |
|---|---|
| Raw Kafka events | Topic `us-stocks-raw` (6 partitions, 7-day retention) |
| Invalid events | Topic `us-stocks-dead-letter` (30-day retention) |
| Clean Parquet (per ticker) | `s3a://stocks/clean/<TICKER>/` — browse at http://localhost:9001 |
| Windowed aggregations | `s3a://stocks/aggregated/window_date=YYYY-MM-DD/` |
| Spark checkpoints | `s3a://stocks/checkpoints/` |
| Analytics tables (10) | PostgreSQL `stocks_analytics` — connect at `localhost:5432` |
| Analytics views (8) | PostgreSQL `stocks_analytics` — `v_top_gainers`, `v_market_trend`, etc. |
